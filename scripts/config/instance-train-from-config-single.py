...
class CONFIG:
    ...
    def __init__(self, config):
        ...
        self.training_rois = [fix_file_paths_linux_windows(fp) for fp in self.training_rois if fp]
        if self.valid_images:
            self.valid_images = [
                fix_file_paths_linux_windows(fp) for fp in self.valid_images if fp
            ]
        if self.valid_rois:
            self.valid_rois = [fix_file_paths_linux_windows(fp) for fp in self.valid_rois if fp]
...
if __name__ == "__main__":
    ...
    MetadataCatalog.get("train_data").set(thing_classes=[train_data.class_data.keys()])
    DatasetCatalog.register("val_data", test_data.get_dict)
    MetadataCatalog.get("val_data").set(thing_classes=[train_data.class_data.keys()])

    cfg_d2 = get_cfg()

    cfg_d2.merge_from_file(model_zoo.get_config_file(cfg.architecture))
    ...
    cfg_d2.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE = 256
    num_classes = len(train_data.class_data.keys())

...
if __name__ == "__main__":
    ...
    if not cfg.reload_from_pickle:
        for train_im, train_anno, roi_anno in zip(
            ...
            for area_idx in range(n_train_areas):
                ...
                wmap.intersect_tiles_annos(exclusion_roi=["train-area"])
                ...
                def hash_obj(
                    data: ty.Union[
                        ty.Iterable, ty.List, ty.Dict, ty.Tuple, str, int, float
                    ]
                ) -> str:
                    """Hash python object"""
                    hash_id = hashlib.md5()
        ...
        if not Path(data_dir).exists():
            print(f"making directory at {Path(data_dir).as_posix()}")
            Path(data_dir).mkdir(exist_ok=True)
        data_dir = Path("C:/temp/AF-multi-test")
        for idx, inst in enumerate(tqdm(train_datai)):
            out_file = data_dir / f"{inst['hash']}.tiff"
    ...
    cfg_d2.train.max_iter = len(train_datai) * 25
    # print(cfg_d2.train.max_iter)
    cfg_d2.lr_multiplier.scheduler.milestones = [
        737498,
        737499,
    ]  # int(cfg_d2.train.max_iter*0.87)*100]
